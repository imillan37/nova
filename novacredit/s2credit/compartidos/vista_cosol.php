...
$sql = "SELECT solicitud_cosol.ID_Solicitud  ,
CONCAT(solicitud.Nombre,' ',solicitud.NombreI,' ',solicitud.Ap_paterno,' ',solicitud.Ap_materno)
FROM solicitud_cosol
left join solicitud on solicitud_cosol.ID_Solicitud = solicitud.ID_Solicitud
WHERE solicitud_cosol.ID_Cosol='$Param2'";
$rs=$db->Execute($sql);
...
echo"<CENTER><H2>CONSULTA DE COSOLICITANTE.</H2></CENTER>";
...
$sql_cons ="SELECT  Fecha, Atendio, Status, Suceso
            FROM cosolicitante_sucesos
            WHERE ID_Cosol = '".$Param2."' ORDER BY  Fecha";
