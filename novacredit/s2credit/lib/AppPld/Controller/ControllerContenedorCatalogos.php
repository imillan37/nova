...
class ControllerContenedorCatalogos {
    ...
    public $muestra_global = 10;
        ...
        $QueryWhere = "";
 		...
 		public function AddPersonas($arrDatos,$ID_USR){
            ...
            //***********modificacion por RLJ para el curp**********//
	 		...
	 		$Query = "INSERT INTO pld_cat_nombres_ppe(
	 		                      ID_Usr,
	 		                      Nombre_I,
	 		                      Ap_paterno,
	 		                      Ap_materno,
	 		                      ...
	 		                      Ap_paterno_soundex)
	 					  VALUES( ".$ID_USR.",
	 					          '".mb_strtoupper($arrDatos[0])."',
	 					          '".mb_strtoupper($arrDatos[1])."',
	 					          '".mb_strtoupper($arrDatos[2])."', 
	 					          '".mb_strtoupper($arrDatos[3])."',
	 					          '".mb_strtoupper($arrDatos[4])."',
	 					          
	 					          '".mb_strtoupper($this->MySoundex->get_soundex($arrDatos[0]))."',
	 					          '".mb_strtoupper($this->MySoundex->get_soundex($arrDatos[1]))."',
	 					          '".mb_strtoupper($this->MySoundex->get_soundex($arrDatos[2]))."'  )";
		 	...
		 	if( !empty($id) ){
			 	...
			 	$Query = "UPDATE pld_cat_nombres_ppe 
			 				 SET Nombre_I           = '".mb_strtoupper($arrDatos[0])."',
			 				     ...
			 				     Nombre_I_soundex   = '".mb_strtoupper($this->MySoundex->get_soundex($arrDatos[0]))."',
			 				     Ap_materno_soundex = '".mb_strtoupper($this->MySoundex->get_soundex($arrDatos[1]))."',
			 				     Ap_paterno_soundex = '".mb_strtoupper($this->MySoundex->get_soundex($arrDatos[2]))."'
	 	...
	 	public function DeletePersonas($id,$ID_USR){
		 	...
		 	if(!empty($id)){
                ...
                $QueryRespaldo = "INSERT INTO pld_cat_nombres_ppe(
                                              ID_Nmb_ppe,
                                              ID_Usr,
                                              Nombre_I,
                                              Ap_paterno,
									  ...
									  VALUES( ".$ID_Nmb_ppe.",
									          ".$ID_Usr.",
									          '".mb_strtoupper($Nombre_I)."',
									          '".mb_strtoupper($Ap_paterno)."',
									          '".mb_strtoupper($Ap_materno)."',
									          '".mb_strtoupper($RFC)."',
									          '".mb_strtoupper($CURP)."',
	    ...
	    public function AddFormDatosPersonas($id){	
	 	...
	 	public function EliminarCP($id,$ID_USR){
    ...
    //*****************RLJ esto es para las nuevas entidades de listas y condusef
        ...
        public function AddListasPropias($arrDatos,$ID_USR){
            ...
            if( empty($ID_Nmb_lp) ){
                ...
                $Query = "INSERT INTO pld_cat_nombres_lp(
                                      ID_Usr,
                                      Nombre_I,
                                      Ap_paterno,
                                      Ap_materno,
        ...
        public function AddSAT($arrDatos,$ID_USR){

            $NombreCompleto = mb_strtoupper($arrDatos[0])." ".mb_strtoupper($arrDatos[1])." ".mb_strtoupper($arrDatos[2]);

            $Query = "SELECT COUNT(pld_cat_nombres_sat.ID_Nmb_sat) AS ID_Nmb_sat
                            FROM pld_cat_nombres_sat
                           WHERE CONCAT(pld_cat_nombres_sat.Nombre_I,' ',pld_cat_nombres_sat.Ap_paterno,' ',pld_cat_nombres_sat.Ap_materno)  = '$NombreCompleto' ";

            $RESPUESTA      = $this->db->Execute($Query); // debug($Query);
            $ID_Nmb_lp       = $RESPUESTA->fields["ID_Nmb_sat"];


            if( empty($ID_Nmb_lp) ){

                $Query = "INSERT INTO pld_cat_nombres_sat(
                                      ID_Usr,
                                      Nombre_I,
                                      Ap_paterno,
                                      Ap_materno,
            ...
            if( !empty($id) ){
                ...
                $Query = "UPDATE pld_cat_nombres_lp
                                     SET Nombre_I           = '".mb_strtoupper($arrDatos[0])."',
                                         ...
                                         Nombre_I_soundex   = '".mb_strtoupper($this->MySoundex->get_soundex($arrDatos[0]))."',
                                         Ap_paterno_soundex = '".mb_strtoupper($this->MySoundex->get_soundex($arrDatos[1]))."',
                                         Ap_materno_soundex = '".mb_strtoupper($this->MySoundex->get_soundex($arrDatos[2]))."'
        ...
        public function EditionSAT($arrDatos,$ID_USR,$id){

            //***********modificacion por RLJ para el curp**********//
            if( !empty($id) ){

                $Query = "UPDATE pld_cat_nombres_sat
                                     SET Nombre_I           = '".mb_strtoupper($arrDatos[0])."',
                                         Ap_paterno         = '".mb_strtoupper($arrDatos[1])."',
                                         Ap_materno         = '".mb_strtoupper($arrDatos[2])."',
                                         RFC                = '".mb_strtoupper($arrDatos[3])."',
	    ...
	    public function VistaSAT($BuscaRFC,$Evento,$Pagina){
            ...
            if( !empty($BuscaRFC) ){
                $BuscaRFC = str_replace(" ", "%", $BuscaRFC);
                $QueryWhere = "WHERE CONCAT(pld_cat_nombres_sat.Nombre_I,' ',pld_cat_nombres_sat.Ap_paterno,' ',pld_cat_nombres_sat.Ap_materno) LIKE '%".$BuscaRFC."%'";
            } // fin if

            $Query_count = "SELECT count(*) as Total
									FROM pld_cat_nombres_sat $QueryWhere ";
            ...
            $Query = "SELECT
                             pld_cat_nombres_sat.ID_Nmb_sat,
                             pld_cat_nombres_sat.ID_Usr,
                             pld_cat_nombres_sat.Nombre_I,
                             pld_cat_nombres_sat.Ap_paterno,
                             pld_cat_nombres_sat.Ap_materno,
                             pld_cat_nombres_sat.RFC,
                             pld_cat_nombres_sat.CURP,
                             pld_cat_nombres_sat.Fecha_sistema
                        FROM pld_cat_nombres_sat
                        $QueryWhere
                        ORDER BY ID_Nmb_sat DESC
                        $limite ";
            ...
            while( !$RESPUESTA->EOF ) {
                ...
                $htmlTbody .= '
                    ...
                    <tr>
                        ...
                        <td>'.mb_strtoupper($CURP).'</td>
                        <td>'.mb_strtoupper($Fecha_sistema).'</td>
                        <td align="center"><a class="ui mini green button" onclick="EditarSAT('.$ID_Nmb_lp.');">Editar</a></td>
                        <td align="center"><a class="ui mini red button" onclick="EliminarSAT('.$ID_Nmb_lp.');">Eliminar</a></td>
                    </tr>
...
//*****************RLJ esto es para las nuevas entidades de listas y condusef
    ...
    public function AddListasCondusef($arrDatos,$ID_USR){
        ...
        if( empty($ID_Nmb_lc) ){
            ...
            $Query = "INSERT INTO pld_cat_nombres_lc(
                                  ID_Usr,
                                  Nombre_I,
                                  Ap_paterno,
                                  Ap_materno,
    ...
    public function DeleteListasPropias($id,$ID_USR){
        ...
        if(!empty($id)){
            ...
            $QueryRespaldo = "INSERT INTO pld_cat_nombres_lp(
                                          ID_Nmb_lp,
                                          ID_Usr,
                                          Nombre_I,
                                          Ap_paterno,
								  ...
								  VALUES( ".$ID_Nmb_lp.", 
								          ...
								          '".mb_strtoupper($this->MySoundex->get_soundex($Nombre_I))."',
								          '".mb_strtoupper($this->MySoundex->get_soundex($Ap_paterno))."',
								          '".mb_strtoupper($this->MySoundex->get_soundex($Ap_materno))."' ) ";
    ...
    public function DeleteSAT($id,$ID_USR){
        ...
        if(!empty($id)){
            ...
            $QueryRespaldo = "INSERT INTO pld_cat_nombres_sat (ID_Nmb_lp,ID_Usr,Nombre_I,Ap_paterno,Ap_materno,RFC,CURP)
            ...
            $Query = "DELETE FROM pld_cat_nombres_sat WHERE ID_Nmb_sat = '".$id."'";
            $this->db->Execute($Query);  //debug($Query);

            $QueryRespaldo=str_replace("'","\'",$QueryRespaldo);
            $Query = "INSERT INTO pld_cat_listas_negras_log (ID_persona,ID_Usr,Tipo,Accion,sentencia)
	 					  			   VALUES
	 					  		   		  (".$id.", ".$ID_USR.", 'SAT', 'DELETE','".$QueryRespaldo."')";
            $this->db->Execute($Query); //debug( $Query );
