...
//	debug(" id_sucursal $id_sucursal");
	...
	$sql = "SELECT   clientes.Regimen
		FROM     clientes
		WHERE    clientes.Num_cliente  = '".$num_cliente."' ";      
        ...
        $sql = "SELECT                  cat_productosfinancieros.TasaMensual, 
					...
					cat_productosfinancieros.Periodos_con_Gracia
	...
	$this->periodos_con_gracia    	= ($rs->fields['Periodos_con_Gracia']=='Si')?(true):(false);	
...
*/    
        ...
        for ($i=0; $i<=$this->plazo; $i++)
	{
								...
								if($this->periodos_con_gracia)
								{
									 if($i== $this->plazo)
									 {
										$abono_capital = $saldo_capital;
										$_renta = ( $abono_capital + $abono_comision + $abono_interes + $abono_iva + $abono_segv + $abono_segd + $abono_segb ); 					 	 
									 }
									 else
									 {	
										$abono_capital = 0;
										$_renta =($abono_comision + $abono_interes + $abono_iva + $abono_segv + $abono_segd + $abono_segb ); 					 	
									 }
								}
								else
								{
...
function cotiza_con_cuotas_extra()
{
}
...
function verifica_status_cliente()
{
 	...
 	if($_SESSION['DB_EMP']=="siuc_v45")
 	{


		$sql="  SELECT fact_cliente.id_factura, 
			       fact_cliente.fecha_exp,
				 grupo_solidario_integrantes.ID_grupo_soli,
				 IF(grupo_solidario_pago_anticipado.Fecha IS NULL,'N','Y') AS SKIP                       
			...
			LEFT JOIN grupo_solidario_pago_anticipado	ON  grupo_solidario_integrantes.ID_grupo_soli = grupo_solidario_pago_anticipado.ID_grupo_soli
									AND grupo_solidario_integrantes.Ciclo_gpo     = grupo_solidario_pago_anticipado.Ciclo_gpo
...
$PERIODOS_GRACIA =($this->periodos_con_gracia )?("Si"):("No"); 
...
$zona = (($this->iva_general * 100) >= 15)?('A'):('B');
                    ...
                    $sql="INSERT INTO fact_cliente 
                        ...
                        ( SELECT 
                                 ...
                                 RedondeoCifras,
                                 MoratorioMinimo,
                                 SUBSTR(Nombre,1,50) AS Nombre_Producto,
                                 Gastos_Cobranza_Porcentaje,
                                 Extemporaneos_Tipo
                                ...
                                $_leyenda     = "Cuota ".$this->tipo_plazox ." no. ".$i." de ".$this->plazo." ";
                                                ...
                                                $sql .= "\n('".($ID)."',
                                                 ...
                                                 $sql .= "  '".gfecha($row['FECHA'])."',
                                                            ...
                                                            '".number_format( $row['ABONO_IVA_INTERES']		,4,".","")."',                                                             

                                                            '".number_format( $row['COMISION']     		,4,".","")."',
                                                            '".number_format( $row['ABONO_IVA_COMISION']	,4,".","")."',
                                                                                                                        
                                                            '".number_format( $row['ABONO_SEGV']   		,4,".","")."', 
                                                            '".number_format( $row['ABONO_IVA_SEGV']  		,4,".","")."',                                                             
                                                                                                                  
                                                            '".number_format( $row['ABONO_SEGD']   		,4,".","")."', 
                                                            '".number_format( $row['ABONO_IVA_SEGD']    	,4,".","")."',                                                             
                                                            
                                                            '".number_format( $row['ABONO_SEGB']   		,4,".","")."', 
                                                            '".number_format( $row['ABONO_IVA_SEGB'] 		,4,".","")."',                                                             
...
function get_fecha_vencimiento($num_cliente,$capital,$id_producto,$plazo,$id_sucursal,$Credito_Fecha_Primer_Vencimiento)
{								
  global $db;
  global $ID_SUC;

  $genera= new TNuevoCredito($num_cliente, date("Y-m-d"), date("Y-m-d"),   $capital, $id_producto, $plazo, $db, $ID_SUC,'');
