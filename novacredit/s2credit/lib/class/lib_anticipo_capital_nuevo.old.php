   ...
   //-----------------------------------------------------
	...
	if($rs->_numOfRows)
	    ...
	    while(! $rs->EOF)
	    {
            ...
            if(($rs->fields['SegV']*1)>0)
            {
                $pcnt_iva_segv = $rs->fields['IVA_SegV']*1/$rs->fields['SegV']*1;
            }
	   ...
	   while( $this->tabla_amortizacion[$i]['SALDO_CAPITAL'] > 0)
	   {
	   	...
	   	if($ANTICIPO_APLICADO)
	   	{
				...
				$abono_iva_segv = ($this->cuota_segv * $pcnt_iva_segv ) ; 
...
/**/
	   ...
	   $err_diff = ($this->tot_abono_capital - $this->capital);
	   
	   if( $err_diff  >= 0.01)
	   {
	   	++$this->error;
	   	
	   	error_msg("Error : Monto de anticipo a capital inválido ( ".number_format($err_diff,2) .")");
