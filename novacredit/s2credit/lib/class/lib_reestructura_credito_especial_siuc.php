    ...
    if( $this->status == 0)
    {
        ...
        if($this->genera->no_generar == 0) 
        {
               ...
               $this->aplica_vencimento_anticipado();
                ...
                //debug(" if(this->obj_credito->adeudo_total [".$this->obj_credito->adeudo_total."] < 0.005) ");
                if($this->obj_credito->adeudo_total < 0.005)
                {

			
			if($this->capital_autorizado > 0)
			{
                        ...
                        else
                        {
                           //Como no se generó el nuevo crédito, le damos Rollback al vencimiento anticipado.
                            ...
                            /*    
                                ...
                                $sql = "DELETE FROM notas_credito WHERE ID_Concepto IN( -30,-31,-32,-33,-34, -35) and  num_compra='".$this->num_compra."' ";
                                $this->db->Execute($sql);
                             */
...
function genera_nuevo_credito()
{
                        ...
                        else
                        {
                                if($this->capital_autorizado > 0)
                                {
                                	...
                                	$this->genera->genera_credito_nuevo($id_fondeo, $id_asoc_compra,  '');
                                	
                                
                                	$this->id_factura_destino = $this->genera->id_factura;
                                	
                                	$response = $this->id_factura_destino;
                                }
                                else
                                {
                                	$response = 1;
...
function aplica_vencimento_anticipado()
{
        ...
        if( $this->status >= 0)
        {
            ...
            //debug("this->error_msg  ".$this->error_msg );
