...
function inicializa()
{
	...
	{
		error_msg("1) Una de las fechas del periodo es inv�lida (".$dd."/".$mm."/".$yy.") ");
		die("</BODY></HTML>");
	...
	{
		error_msg("2) Una de las fechas del periodo es inv�lida (".$_dd."/".$_mm."/".$_yy.") ");
		die("</BODY></HTML>");
	...
	if($rs->fields[0] != $_SESSION['ID_USR'] && $_SESSION['ID_USR'] != -21)
	{
		error_msg(" Solo el Oficial de cumpliento est� autorizado para utilizar este m�dulo. ");
		die("</BODY></HTML>");
	}
	
	//-----------------------------------------------------------------------------------------------------
	// Obtener los par�metros para las alertas
	//-----------------------------------------------------------------------------------------------------
	...
	//-----------------------------------------------------------------------------------------------------
	// Verificar que el periodo de tiempo sea v�lido
	//-----------------------------------------------------------------------------------------------------
	...
	for($i=$this->ff_ini; $i<$this->ff_fin; $i=date("Y-m-d",mktime(0,0,0,$mm,1,$yy)))
	{
		...
		if(($num_periodos > 0) and ($num_periodos < 13))
		{
			...
			$this->aPeriodos[] = " SUM(IF(pagos.Fecha BETWEEN '".$i."' and '".date("Y-m-t",mktime(0,0,0,$mm,1,$yy))."', pagos.Monto, 0)) As Periodo_".$strNum;

			$this->aUltimoDiaPeriodo[]  = date("Y-m-t",mktime(0,0,0,$mm,1,$yy));


			//----------------------------------------------------------------------------------------------		
			// Inicializamos los valores de fechas que requerimos para el hist�rco de cambio de divisas
			//----------------------------------------------------------------------------------------------		
	...
	if($num_periodos == 0)
	{
		error_msg("El periodo m�nimo que se puede analizar es de 1 mes.");
		die("</BODY></HTML>");
	...
	if($num_periodos > 12)
	{
		error_msg("El periodo m�s grande que se puede analizar es de 12 meses.");
		die("</BODY></HTML>");
	}
	...
	//----------------------------------------------------------------------------------------------		
	// Validamos que existan los tipos de cambio de divisa en las fechas indicadas para poder realizar los c�lculos de 
	// conversi�n a d�lares.
	//
	// Se usa la fecha con la que cerr� el mes anterior
	//----------------------------------------------------------------------------------------------		
	...
	$sql = "SELECT pld_importacion_tipocambio.Dia,
		       pld_importacion_tipocambio_dtl.Valor_venta
		...
		WHERE pld_importacion_tipocambio.ID_Importacion = pld_importacion_tipocambio_dtl.ID_Importadtl
			AND pld_importacion_tipocambio.Dia IN ('".$listado_fechas."') 
	 ...
	 if($rs->_numOfRows)
	    while(! $rs->EOF)
	    {
			...
			$_FechaCierreDivisa = $rs->fields['Dia'];

			$this->tipo_cambio_usd[$_FechaCierreDivisa] = $rs->fields['Valor_venta']*1; 

			$this->tipo_cambio_usd_ord[] = $rs->fields['Valor_venta']*1; 
	...
	$error = 0;
	$error_msg ="";
	foreach($this->tipo_cambio_usd AS $fecha => $valor)
	{
		if($valor==0) 
		{

			$error++;
			$error_msg .= "<SPAN STYLE='color:red;'>".$error.") Unidad de cambio al ".ffecha($fecha)."</SPAN><br>\n";
	...
	if($error > 0)
	{
	  ...
	  $msg  = "Error : Se requiere el valor de cambio del USD en las fechas especificadas <br>";
	  $msg .= "para pder continuar.<br><br>";
	  $msg .= "Ingrese los valores solicitados en el cat�logo de TIPO DE CAMBIO USD<br>";
	  $msg .= "En la secci�n de CATALOGOS PLD "."<DIR>".$error_msg."</DIR>";
...
function mineria_datos_pld()
{
	...
	//---------------------------------------------------------------------------------------------------
	// Gener�mos la consulta en forma din�mica para que ponga los vlaores de los periodos en orden
	//===================================================================================================
	...
	{	
		...
		$this->aPeriodos[] = " 0.0 AS Periodo_".$strNum;
	}
			    ...
			    (
					SELECT fact_cliente.ID_Factura		AS ID_Factura,
						1 				AS ID_Mineria,
					       ...
					       MIN(pagos.Fecha)     		AS Primer_Pago,
					       MAX(pagos.Fecha)     		AS Ultimo_Pago,
					       COUNT(pagos.ID_Pago) 		AS Num_Total_Pagos,
					       SUM(pagos.Monto)     		AS Monto_Periodo, \n";
	...
	$sql_concentrado .= " 		FROM fact_cliente
					...
					INNER JOIN pagos    ON  pagos.Num_compra     = fact_cliente.num_compra 
					                    AND pagos.activo = 'S'
	...
	$sql = " UPDATE pld_mineria_pagos_credito
		    SET pld_mineria_pagos_credito.MAX_Monto_Periodo 
		      = GREATEST(Periodo_01, Periodo_02, Periodo_03, Periodo_04, Periodo_05, Periodo_06, 
				 Periodo_07, Periodo_08, Periodo_09, Periodo_10, Periodo_11, Periodo_12) ";
	...
	$sql = " UPDATE pld_mineria_pagos_credito
		 SET    pld_mineria_pagos_credito.MAX_Num_Periodo = 
		     ...
		     CASE pld_mineria_pagos_credito.MAX_Monto_Periodo   
			     ...
			     WHEN pld_mineria_pagos_credito.Periodo_01 THEN 01
			     WHEN pld_mineria_pagos_credito.Periodo_02 THEN 02
			     WHEN pld_mineria_pagos_credito.Periodo_03 THEN 03
			     WHEN pld_mineria_pagos_credito.Periodo_04 THEN 04
			     WHEN pld_mineria_pagos_credito.Periodo_05 THEN 05
			     WHEN pld_mineria_pagos_credito.Periodo_06 THEN 06
			     WHEN pld_mineria_pagos_credito.Periodo_07 THEN 07
			     WHEN pld_mineria_pagos_credito.Periodo_08 THEN 08   
			     WHEN pld_mineria_pagos_credito.Periodo_09 THEN 09   
			     WHEN pld_mineria_pagos_credito.Periodo_10 THEN 10   
			     WHEN pld_mineria_pagos_credito.Periodo_11 THEN 11   
			     WHEN pld_mineria_pagos_credito.Periodo_12 THEN 12 
		     END ";
	...
	for($i=1; $i <= $this->num_periodos; $i++)
	{
		...
		$sql = " UPDATE pld_mineria_pagos_credito
			...
			(
				...
				INNER JOIN pagos        ON pagos.Num_compra = fact_cliente.num_compra AND pagos.activo = 'S'

				WHERE pagos.Fecha BETWEEN '".$fini."' and '".$ffin."'
	...
	$sql="  INSERT INTO pld_mineria_pagos_cliente_usd
		(
			 ID_Cliente,     ID_Mineria,     Num_Cliente,    Regimen,	Primer_Pago,    Ultimo_Pago,    Num_Total_Pagos,	Num_Total_Creditos,
		...
		(
			...
			SELECT  pld_mineria_pagos_credito.ID_Cliente			AS ID_Cliente	,
				...
				SUM(pld_mineria_pagos_credito.Periodo_01)		AS Periodo_MX_01,
				SUM(pld_mineria_pagos_credito.Periodo_02)		AS Periodo_MX_02,
				SUM(pld_mineria_pagos_credito.Periodo_03)		AS Periodo_MX_03,
				SUM(pld_mineria_pagos_credito.Periodo_04)		AS Periodo_MX_04,
				SUM(pld_mineria_pagos_credito.Periodo_05)		AS Periodo_MX_05,
				SUM(pld_mineria_pagos_credito.Periodo_06)		AS Periodo_MX_06,
				SUM(pld_mineria_pagos_credito.Periodo_07)		AS Periodo_MX_07,
				SUM(pld_mineria_pagos_credito.Periodo_08)		AS Periodo_MX_08,
				SUM(pld_mineria_pagos_credito.Periodo_09)		AS Periodo_MX_09,
				SUM(pld_mineria_pagos_credito.Periodo_10)		AS Periodo_MX_10,
				SUM(pld_mineria_pagos_credito.Periodo_11)		AS Periodo_MX_11,
				SUM(pld_mineria_pagos_credito.Periodo_12)		AS Periodo_MX_12,

				
				SUM(pld_mineria_pagos_credito.Periodo_01)*(IF((pld_mineria_datos.Tipo_Cambio_01>0),(1/pld_mineria_datos.Tipo_Cambio_01),0))	AS Periodo_USD_01,
				SUM(pld_mineria_pagos_credito.Periodo_02)*(IF((pld_mineria_datos.Tipo_Cambio_02>0),(1/pld_mineria_datos.Tipo_Cambio_02),0))	AS Periodo_USD_02,
				SUM(pld_mineria_pagos_credito.Periodo_03)*(IF((pld_mineria_datos.Tipo_Cambio_03>0),(1/pld_mineria_datos.Tipo_Cambio_03),0))	AS Periodo_USD_03,
				SUM(pld_mineria_pagos_credito.Periodo_04)*(IF((pld_mineria_datos.Tipo_Cambio_04>0),(1/pld_mineria_datos.Tipo_Cambio_04),0))	AS Periodo_USD_04,
				SUM(pld_mineria_pagos_credito.Periodo_05)*(IF((pld_mineria_datos.Tipo_Cambio_05>0),(1/pld_mineria_datos.Tipo_Cambio_05),0))	AS Periodo_USD_05,
				SUM(pld_mineria_pagos_credito.Periodo_06)*(IF((pld_mineria_datos.Tipo_Cambio_06>0),(1/pld_mineria_datos.Tipo_Cambio_06),0))	AS Periodo_USD_06,
				SUM(pld_mineria_pagos_credito.Periodo_07)*(IF((pld_mineria_datos.Tipo_Cambio_07>0),(1/pld_mineria_datos.Tipo_Cambio_07),0))	AS Periodo_USD_07,
				SUM(pld_mineria_pagos_credito.Periodo_08)*(IF((pld_mineria_datos.Tipo_Cambio_08>0),(1/pld_mineria_datos.Tipo_Cambio_08),0))	AS Periodo_USD_08,
				SUM(pld_mineria_pagos_credito.Periodo_09)*(IF((pld_mineria_datos.Tipo_Cambio_09>0),(1/pld_mineria_datos.Tipo_Cambio_09),0))	AS Periodo_USD_09,
				SUM(pld_mineria_pagos_credito.Periodo_10)*(IF((pld_mineria_datos.Tipo_Cambio_10>0),(1/pld_mineria_datos.Tipo_Cambio_10),0))	AS Periodo_USD_10,
				SUM(pld_mineria_pagos_credito.Periodo_11)*(IF((pld_mineria_datos.Tipo_Cambio_11>0),(1/pld_mineria_datos.Tipo_Cambio_11),0))	AS Periodo_USD_11,
				SUM(pld_mineria_pagos_credito.Periodo_12)*(IF((pld_mineria_datos.Tipo_Cambio_12>0),(1/pld_mineria_datos.Tipo_Cambio_12),0))	AS Periodo_USD_12,
...
function buscar_actividad_inusual()
{
	...
	//---------------------------------------------------------------------------------------
	// Cr�ditos cuyos pagos exceden mucho el porcentaje de pago respecto a su couta  
	//---------------------------------------------------------------------------------------
       ...
       $sql="  SELECT    clientes.ID_Cliente,
		...
		WHERE (pld_mineria_pagos_credito.Monto_Cuota_Mensualizada * ".($this->Pcnt_Minimo_Inusual_Rebasa_Pagos/100).") <= pld_mineria_pagos_credito.MAX_Monto_Periodo ";
	...
	if($rs->_numOfRows)
	   while(! $rs->EOF)
	   {
		...
		$alerta[$ID]['MOTIVO'] 			= "El cliente presenta pagos acumulados que exceden por ".number_format(($this->Pcnt_Minimo_Inusual_Rebasa_Pagos/100),2)." veces o m�s la cuota mensual de su cr�dito.";
	...
	//---------------------------------------------------------------------------------------
	// Cr�ditos que entre su perfil transaccional m�s reciente y el anterior aumentraon mucho 
	// sus ingresos  
	//---------------------------------------------------------------------------------------
	...
	//---------------------------------------------------------------------------------------
	// Motiva sospechas de actividades il�citas
	//---------------------------------------------------------------------------------------
...
// $this->Minimo_Relevantes_Fisicas_USD = 1000;
	...
	if($rs->_numOfRows)
	   while(! $rs->EOF)
	   {
		...
		{
			$alerta[$ID]['MOTIVO'] = "Persona moral, con pagos acumulados mensuales por ".(1 *$this->Minimo_Relevantes_Morales_USD)." USD o m�s";
		}
		else
		{
			$alerta[$ID]['MOTIVO'] = "Persona f�sica, con pagos acumulados mensuales por ".(1 *$this->Minimo_Relevantes_Fisicas_USD)." USD o m�s";
		}
