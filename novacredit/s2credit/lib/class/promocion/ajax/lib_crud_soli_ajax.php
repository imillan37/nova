...
function insert_campos_solicitud(){
  ...
  if($db->_insertid() > 0)
  {
	...
	/*******RENUEVA SOLICITUD*******/
	 ...
	 //************************************* START VALIDACION PLD******************************************

	 	require_once( "../../libValidacionOriginacion.php" ); 
	                   
	    $OBJ = new libValidacionOriginacion($db,$ID_RESULT_SOLI,$ID_USR);
	    $arrResultados = $OBJ->resultadosConfiguracion();
	            
	    $total_riesgos = 0;
	    $msj_alertas = "";
	    foreach($arrResultados as $valor)
	    {
	        if($valor != "")
	        {
	            $msj_alertas .= $valor." \n\n ";
	    ...
	    if($total_riesgos > 0){
	        ...
	        $qry_oficial = "SELECT
	                        usuarios.email as Correo_oficial
	                      ,pld_parametros_configuracion.Correo_oficial as Correo_alternativo
	                    FROM pld_parametros_configuracion
	                    INNER JOIN usuarios ON usuarios.ID_User =  pld_parametros_configuracion.ID_User_Oficial_Cumplimiento ";
	        $RESPUESTA = $db->Execute($qry_oficial);
	        $Correo_oficial      = $RESPUESTA->fields["Correo_oficial"];
	        $Correo_alternativo  = $RESPUESTA->fields["Correo_alternativo"];
	        
	        if($Correo_oficial != "" or $Correo_alternativo != "")
	        {
	            //@Nombre solicitud
	            $qry_nombre = "SELECT
                                     IF(
	                                    solicitud_pmoral.ID_Solicitud IS NOT NULL,
	                                    solicitud_pmoral.Razon_social,
	                                    CONCAT( solicitud.Nombre,' ',solicitud.NombreI,' ',solicitud.Ap_paterno,' ',solicitud.Ap_materno ) ) AS nombreSolicitud
                                   FROM solicitud
                              LEFT JOIN solicitud_pmoral ON solicitud_pmoral.ID_Solicitud = solicitud.ID_Solicitud
                                  WHERE solicitud.ID_Solicitud = '".$ID_RESULT_SOLI."' ";
	            
	                $response = $db->Execute($qry_nombre);
	                $nombreSolicitud = $response->fields["nombreSolicitud"];
	            
	            try{
	                include('../../../e-mail/PHPMailerAutoload.php');
	                
	                $mail = new PHPMailer;
					$mail->isSMTP();
					$mail->SMTPDebug = 0;
					$mail->Debugoutput = 'html';
					$mail->Host = "209.213.117.238";
					$mail->Port = "587";
					$mail->SMTPAuth = true;
					$mail->Username = "alertas@s2credit.com";
					$mail->Password = "2e75fb369e35b855864794d1622d4a4b";
					$mail->setFrom( "alertas@s2credit.com" , "Alertas P.L.D" );
					$mail->Subject = "Alerta de oficial de cumplimiento";
					
					$mail->addAddress( $Correo_oficial , "Oficial Cumplimiento" );
					$mail->addAddress( $Correo_alternativo , "Oficial Cumplimiento" );
					$mail->addAddress( "andres.cruz@s2credit.com" , "Oficial Cumplimiento" );

					$body  = "Estimado Oficial de cumplimiento le notificamos que la siguiente solicitud <b>#".$ID_RESULT_SOLI."</b> bajo el nombre de ".$nombreSolicitud.", se ha detenido por considerarse riesgosa. \n \n ".$msj_alertas." \n\n Favor de darle seguimiento  \n Gracias. ";
...
function update_campos_solicitud()
{
 ...
 global $fechas_captura;
    ...
    $OBJ = new libValidacionOriginacion($db,$_POST["Param2"],$ID_USR);
    $arrResultados = $OBJ->resultadosConfiguracion();
    /*
    echo "<pre>";
    ...
    if($total_riesgos > 0){
        ...
        if($Correo_oficial != "" or $Correo_alternativo != "")
        {
            ...
            $qry_nombre = "SELECT
                                   ...
                                   FROM solicitud
                              LEFT JOIN solicitud_pmoral ON solicitud_pmoral.ID_Solicitud = solicitud.ID_Solicitud
                                  WHERE solicitud.ID_Solicitud = '".$_POST["Param2"]."' ";
            ...
            try{
				...
				$body  = "Estimado Oficial de cumplimiento le notificamos que la siguiente solicitud <b>#".$_POST["Param2"]."</b> bajo el nombre de ".$nombreSolicitud.", se ha detenido por considerarse riesgosa. \n \n ".$msj_alertas." \n\n Favor de darle seguimiento  \n Gracias. ";
